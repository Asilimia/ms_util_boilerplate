...
async def initialize_redis_connection(backend_app: fastapi.FastAPI) -> None:
    ...
    backend_app.state.redis = aioredis.from_url(
        url=backend_app.state.settings.REDIS_URL,
        encoding="utf-8",
...
async def close_redis_connection(backend_app: fastapi.FastAPI) -> None:
    loguru.logger.info("Redis Connection --- Closing . . .")
    await backend_app.state.redis.close()
    loguru.logger.info("Redis Connection --- Successfully Closed!")
